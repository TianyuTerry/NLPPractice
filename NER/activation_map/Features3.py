...
def str_combine(x, y1, y2):
    return 'combine:' + str(x) + '+' + str(y1) + '+' + str(y2)

def str_combine_pos(x, y1, y2):
    return 'pos_combine:' + str(x) + '+' + str(y1) + '+' + str(y2)
...
def feature2idx(words, tags, postags):
    ...
    for y1, y2 in itertools.product(range(T), range(T)):
        ...
        for x in range(V):
            features.append(str_combine(x, y1, y2))
        for x in range(POS):
            features.append(str_combine_pos(x, y1, y2))
...
def feature_activation(x, tag2index, feature2index):
    ...
    for i, y1, y2 in itertools.product(range(N+1), range(T), range(T)):
        ...
        if i < N and x[i][0] != -1:
            ...
            feature_activation_map[i, y1, y2][feature2index[str_combine(x[i][0], y1, y2)]] = 1
            feature_activation_map[i, y1, y2][feature2index[str_combine_pos(x[i][1], y1, y2)]] = 1
...
def main():
    ...
    path_weight = path/'best_weight_features3.npy'
